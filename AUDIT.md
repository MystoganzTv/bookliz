...
| ~~P2-11~~ *(resuelto 2026-09-12)* | **Vision devolvía 403** (`This API method requires billing to be enabled`) porque la única cuenta de facturación del proyecto estaba cerrada y no era reabrible. Resuelto creando la cuenta de facturación `Bookliz` (USD), vinculándola a `booklio-497503` y verificando la Cloud Vision API activa. `book-vision` probado en producción: HTTP 200, devuelve `isbn` (validado por dígito de control), `title` (bloque tipográfico más alto) y `language` | Cerrado |
