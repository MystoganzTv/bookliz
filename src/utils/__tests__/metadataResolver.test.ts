...
const fetchByIsbn = OLMeta.fetchBookMetadataByIsbn as jest.Mock;
...
beforeEach(() => {
  ...
  fetchByIsbn.mockResolvedValue(undefined);
});
...
describe("resolveMetadata cache TTL", () => {
  ...
  it("does NOT cache when Google Books was rate-limited (result still returned)", async () => {
    ...
    fetchByIsbn.mockResolvedValue({ title: "Dune", authorName: "Frank Herbert", isbn: ISBN, synopsis: LONG_SYNOPSIS });
  ...
  it("does NOT cache when a provider timed out", async () => {
    fetchByKeyword.mockResolvedValue({ books: [gbBook({ description: LONG_SYNOPSIS })], totalItems: 1 });
    fetchByIsbn.mockRejectedValue(new FetchTimeoutError("https://openlibrary.org/isbn/x.json", 10_000));
  ...
  it("asks providers to surface transient failures instead of swallowing them", async () => {
    ...
    expect(fetchByIsbn).toHaveBeenCalledWith(ISBN, { rethrowTransient: true });
  });
